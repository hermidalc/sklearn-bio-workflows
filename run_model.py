...
def calculate_test_scores(
    estimator, X_test, y_test, metrics, predict_params=None, score_params=None
):
    scores = {}
    ...
    for metric in metrics:
        if metric == "roc_auc":
            ...
            scores[metric] = balanced_accuracy_score(y_test, y_pred, **score_params)
        elif metric == "average_precision":
            scores[metric] = average_precision_score(y_test, y_score, **score_params)
...
def run_model():
    ...
    if args.sample_meta_cols:
        ...
        if not pipe_has_penalty_factor:
            feature_meta.drop(columns=[args.penalty_factor_meta_col], inplace=True)
    if groups is not None:
        search_param_routing = {
            "cv": (
                "groups"
                if group_weights is None
                else {"groups": "groups", "weights": "group_weights"}
            ),
            "estimator": [],
            "scoring": [],
        }
    ...
    if groups is None:
        if args.scv_use_ssplit:
            ...
            cv_splitter = RepeatedStratifiedKFold(
                n_splits=args.scv_splits,
                n_repeats=args.scv_repeats,
                random_state=args.random_seed,
            )
        else:
            cv_splitter = StratifiedKFold(
                n_splits=args.scv_splits, random_state=args.random_seed, shuffle=True
            )
    ...
    if args.skb_slr_k_lim:
        min_train_samples = (
            ...
            if args.test_dataset
                ...
                train.size
                for train, _ in test_splitter.split(X, y, **test_split_params)
            )
        )
        for params in param_grid:
        ...
        for param, param_values in param_grid_dict.items():
            param_type = get_param_type(param)
            if param_type in params_k_selected_features:
                param_grid_dict[param] = param_values[param_values <= min_train_samples]
    if args.scv_type == "grid":
        search = ExtendedGridSearchCV(
            pipe,
            cv=cv_splitter,
            error_score=args.scv_error_score,
    ...
    if args.verbose > 0 or args.scv_verbose > 0:
        print(
            "Train:" if args.test_dataset else "Dataset:",
            dataset_name,
            X.shape,
            end=" ",
        )
    ...
    if args.verbose > 0:
        ...
        if (
            ...
            and "sample_weight" in search_param_routing["estimator"]
        ):
    ...
    if args.save_model_code is not None:
        if dataset_name.split("_")[-1] == "eset":
            model_name = "_".join(
                [dataset_name.rpartition("_")[0], args.save_model_code]
            )
        else:
    ...
    if args.test_dataset:
        with parallel_backend(
            args.parallel_backend, inner_max_num_threads=inner_max_num_threads
        ):
        ...
        plot_param_cv_metrics(dataset_name, pipe_name, param_grid_dict, param_cv_scores)
        ...
        for test_dataset in test_datasets:
            ...
            if args.verbose > 0:
                ...
                for metric in args.scv_scoring:
                    print(
                        " {}: {:.4f}".format(metric_label[metric], test_scores[metric]),
                        end=" ",
                    )
                    if metric == "average_precision":
                        print(" PR AUC: {:.4f}".format(test_scores["pr_auc"]), end=" ")
                print()
        # plot top-ranked selected features vs test performance metrics
        if "Weight" in final_feature_meta.columns:
            fig_slr, ax_slr = plt.subplots(figsize=(args.fig_width, args.fig_height))
            fig_slr.suptitle(
                "Effect of Number of Top-Ranked Selected "
            ...
            ax_slr.set_xlabel(
                "Number of top-ranked features selected", fontsize=args.axis_font_size
            )
            ...
            tf_pipe_param_routing = (
                best_pipe.param_routing if best_pipe.param_routing else {}
            )
            tf_pipe_param_routing["slrc"] = pipe_config["ColumnSelector"][
                "param_routing"
            ]
            if isinstance(best_pipe[-1], (RFE, SelectFromUnivariateModel)):
                ...
                if "param_routing" in pipe_config[final_estimator_key]:
                    tf_pipe_param_routing[final_step_name] = pipe_config[
                        final_estimator_key
                    ]["param_routing"]
                else:
            ...
            if args.penalty_factor_meta_col in final_feature_meta.columns:
                ...
                for feature_name in penalized_final_feature_meta.iloc[
                    (-penalized_final_feature_meta["Weight"].abs()).argsort()
                ].index:
                    tf_name_sets.append(
                        tf_name_sets[-1] + [feature_name]
                        if tf_name_sets
                        else [feature_name]
                    )
                tf_name_sets = [
                    feature_names + unpenalized_feature_names
        ...
        if "roc_auc" in args.scv_scoring:
            fig_roc, ax_roc = plt.subplots(figsize=(args.fig_width, args.fig_height))
            fig_roc.suptitle("ROC Curves", fontsize=args.title_font_size)
            ax_roc.set_title(
                "{}\n{}".format(dataset_name, pipe_name),
                fontsize=args.title_font_size - 2,
            )
            ax_roc.set_xlabel("False Positive Rate", fontsize=args.axis_font_size)
            ax_roc.set_ylabel("True Positive Rate", fontsize=args.axis_font_size)
            ax_roc.set_xlim([-0.01, 1.01])
            ax_roc.set_ylim([-0.01, 1.01])
        if "average_precision" in args.scv_scoring:
            fig_pre, ax_pre = plt.subplots(figsize=(args.fig_width, args.fig_height))
        ...
        for test_idx, test_dataset in enumerate(test_datasets):
            ...
            if "Weight" in final_feature_meta.columns:
                ...
                for metric_idx, metric in enumerate(tf_test_scores):
                    ax_slr.plot(
                        ...
                        alpha=0.8,
                        lw=2,
                        color=test_metric_colors[test_idx + metric_idx],
                        label="{} {}".format(test_dataset_name, metric_label[metric]),
                    )
            ...
            if "roc_auc" in args.scv_scoring:
                ax_roc.plot(
                    ...
                    test_scores["tpr"],
                    alpha=0.8,
                    color=test_metric_colors[test_idx * len(args.scv_scoring)],
                    lw=2,
                ...
                ax_roc.plot(
                    ...
                    linestyle="--",
                    lw=2,
                    label=("Chance" if test_idx == len(test_datasets) - 1 else None),
                )
    ...
    # train-test nested cv
        ...
        if args.run_perm_test:
            ...
            for _ in range(args.n_perms):
                ...
                perm_split_idxs.append(
                    list(test_splitter.split(X, perm_ys[-1], **test_split_params))
                )
        ...
        for split_idx, (train_idxs, test_idxs) in enumerate(
            ...
            if groups is not None:
                ...
                if group_weights is not None and pass_cv_group_weights:
                    split_search_fit_params["group_weights"] = group_weights[train_idxs]
            try:
                ...
                with parallel_backend(
                    ...
                    search.fit(
                        X.iloc[train_idxs], y[train_idxs], **split_search_fit_params
                    )
                if pipe_props["uses_rjava"]:
                    ...
                    best_pipe = Parallel(
                        ...
                        for pipe_params in [best_params]
                    )[
                        0
                    ]
                    if args.scv_verbose == 0:
                ...
                split_scores["te"] = calculate_test_scores(
                    ...
                    y[test_idxs],
                    args.scv_scoring,
                    predict_params=split_pipe_predict_params,
                    score_params=split_score_params,
                )
                if args.run_perm_test:
                    if args.perm_verbose > 0:
                        print(
                            "Running permutation test ({:d} permutations)".format(
                                args.n_perms
                            )
                        )
                ...
                if args.verbose > 0:
                    print(
                        "Model:",
                        model_name,
                        " Split: {:>{width}d}".format(
                            split_idx + 1, width=len(str(args.test_splits))
                        ),
                        end=" ",
                        flush=True,
                    )
                ...
                warnings.warn(
                    "Estimator fit/scoring failed. This outer CV "
                    "train-test split will be ignored. Details: {}".format(
                ...
                param_cv_scores = add_param_cv_scores(
                    search, param_grid_dict, param_cv_scores
                )
                final_feature_meta = get_final_feature_meta(best_pipe, feature_meta)
                if args.verbose > 0:
                    print(
                    ...
                    for metric in args.scv_scoring:
                        ...
                        if metric == "average_precision":
                            print(
                                " PR AUC Test: {:.4f}".format(
                                    split_scores["te"]["pr_auc"]
                                ),
                                end=" ",
                            )
                    print(
        ...
        if args.save_results:
            dump(
                split_results, "{}/{}_split_results.pkl".format(results_dir, model_name)
            )
            dump(
                param_cv_scores,
        ...
        for split_result in split_results:
            ...
            for metric in args.scv_scoring:
                if metric not in scores["cv"]:
                    scores["cv"][metric] = []
                    scores["te"][metric] = []
                scores["cv"][metric].append(split_result["scores"]["cv"][metric])
                scores["te"][metric].append(split_result["scores"]["te"][metric])
                if metric == "average_precision":
            ...
            if args.penalty_factor_meta_col in split_feature_meta.columns:
                num_features.append(
                    split_feature_meta.loc[
                        split_feature_meta[args.penalty_factor_meta_col] != 0
                    ].shape[0]
                )
            else:
        ...
        if args.run_perm_test:
            ...
            perm_pvalue = (np.sum(perm_scores >= true_score) + 1.0) / (args.n_perms + 1)
            if args.save_results:
                perm_results = {
                ...
                dump(
                    perm_results,
                    "{}/{}_perm_results.pkl".format(results_dir, model_name),
                )
        print("Model:", model_name, end=" ")
        for metric in args.scv_scoring:
            print(
                " Mean {} (CV / Test): {:.4f} / {:.4f}".format(
                    metric_label[metric],
                    np.mean(scores["cv"][metric]),
                    np.mean(scores["te"][metric]),
                ),
                end=" ",
            )
            if metric == "average_precision":
                print(
                    " Mean PR AUC Test: {:.4f}".format(np.mean(scores["te"]["pr_auc"])),
                    end=" ",
                )
        if num_features and (
            pipe_props["has_selector"] or np.mean(num_features) < X.shape[1]
        ):
            print(" Mean Features: {:.0f}".format(np.mean(num_features)), end=" ")
        if args.run_perm_test:
            print(
                " Permutation Test: True {} = {:.4f} p = {:.4f}".format(
        ...
        for split_idx, split_result in enumerate(split_results):
            ...
            for metric in args.scv_scoring:
                ...
                feature_scores[metric].rename(columns={metric: split_idx}, inplace=True)
            ...
            for feature_annot_col in feature_annots.columns:
                if is_integer_dtype(feature_annots[feature_annot_col]):
                    ...
                    feature_results_floatfmt.append(
                        ".{:d}f".format(
                            max(
                                abs(Decimal(f).as_tuple().exponent)
                                for f in (feature_annots[feature_annot_col].astype(str))
                            )
                        )
                    )
                else:
        ...
        for metric in args.scv_scoring:
            if metric not in ("roc_auc", "balanced_accuracy", "average_precision"):
                raise RuntimeError(
                    "No feature scores fillna value defined " "for {}".format(metric)
                )
                        ...
                        feature_results_floatfmt.append(
                            ".{:d}f".format(
                                max(
                                    abs(Decimal(f).as_tuple().exponent)
                                    for f in (
                                        feature_annots[feature_annot_col].astype(str)
                                    )
                                )
                            )
                        )
            ...
            if feature_scores[metric].mean(axis=1).nunique() > 1:
                feature_results = feature_results.join(
                    pd.DataFrame(
                        {
                            "Mean Test {}".format(metric_label[metric]): feature_scores[
                                metric
                            ].mean(axis=1)
                        }
                    ),
            ...
            if feature_weights is not None:
                print(
                    tabulate(
                        ...
                        floatfmt=feature_results_floatfmt,
                        headers="keys",
                    )
                )
            else:
                print(
                    tabulate(
                        feature_results.sort_values(
                            by="Mean Test {}".format(metric_label[args.scv_refit]),
                            ascending=False,
                        ),
                        floatfmt=feature_results_floatfmt,
                        headers="keys",
        ...
        if "average_precision" in args.scv_scoring:
            ...
            plt.step(
                ...
                label=(
                    ...
                    r"Features = {:.0f} $\pm$ {:.0f})"
                ).format(mean_pr_auc, std_pr_auc, mean_num_features, std_num_features),
            )
        ...
        if args.run_perm_test:
            ...
            plt.xlabel(metric_label[args.scv_refit], fontsize=args.axis_font_size)
            plt.ylabel(args.hist_plot_stat.title(), fontsize=args.axis_font_size)
            plt.tick_params(labelsize=args.axis_font_size)
        ...
        for fig_num in plt.get_fignums():
            ...
            if args.save_figs:
                for fig_fmt in args.fig_format:
                    plt.savefig(
                        "{}/Figure_{:d}.{}".format(results_dir, fig_num, fig_fmt),
                        bbox_inches="tight",
                        format=fig_fmt,
                    )
    if args.show_figs:
...
def add_col_trf_args(args, argv, level=1):
    ...
    parser.add_argument(
        "--col-trf-{}-pipe-steps".format(level),
        type=str_list,
        nargs="+",
        required=True,
        action="append",
        help=("ColumnTransformer {} pipeline step names".format(level)),
    )
    parser.add_argument(
        "--col-trf-{}-patterns".format(level),
        type=str,
        nargs="+",
        help=("ColumnTransformer {} column patterns".format(level)),
    )
    parser.add_argument(
        "--col-trf-{}-dtypes".format(level),
        type=str,
        nargs="+",
        choices=["category", "float", "int"],
        help=("ColumnTransformer {} column dtypes".format(level)),
    )
    parser.add_argument(
        "--col-trf-{}-remainder".format(level),
        type=str,
        choices=["drop", "passthrough"],
        default="passthrough",
        help="ColumnTransformer {} remainder".format(level),
    )
    args, argv = parser.parse_known_args(argv, namespace=args)
    if not (
        hasattr(args, "col_trf_{}_patterns".format(level))
        ...
        parser.error(
            "one of the following arguments is required: {}".format(
                " ".join(
                    [
                        "col_trf_{}_patterns".format(level),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cft-slr-thres", type=float, nargs="+", help="ConfidenceThreshold threshold"
    )
    parser.add_argument(
        "--cft-slr-meta-col",
        type=str,
    ...
    parser.add_argument(
        "--crt-slr-thres", type=float, nargs="+", help="CorrelationThreshold threshold"
    )
    parser.add_argument(
        "--crt-slr-meta-col",
        type=str,
    ...
    parser.add_argument(
        "--mui-slr-n", type=int, nargs="+", help="MutualInfoScorer n_neighbors"
    )
    parser.add_argument(
        "--mui-slr-n-min", type=int, default=1, help="MutualInfoScorer n_neighbors min"
    )
    parser.add_argument(
        "--mui-slr-n-max", type=int, help="MutualInfoScorer n_neighbors max"
    )
    parser.add_argument(
        "--mui-slr-n-step",
        type=int,
    ...
    parser.add_argument("--skb-slr-k", type=int, nargs="+", help="SelectKBest k")
    parser.add_argument(
        "--skb-slr-k-min", type=int, default=1, help="SelectKBest k min"
    )
    parser.add_argument("--skb-slr-k-max", type=int, help="SelectKBest k max")
    parser.add_argument(
        "--skb-slr-k-step", type=int, default=1, help="SelectKBest k step"
    )
    parser.add_argument(
        "--skb-slr-k-lim",
        default=False,
    ...
    parser.add_argument(
        "--sfm-slr-svc-ce", type=int, nargs="+", help="SelectFromModel LinearSVC C exp"
    )
    parser.add_argument(
        "--sfm-slr-svc-ce-min", type=int, help="SelectFromModel LinearSVC C exp min"
    )
    parser.add_argument(
        "--sfm-slr-svc-ce-max", type=int, help="SelectFromModel LinearSVC C exp max"
    )
    parser.add_argument(
        "--sfm-slr-lgr-ce",
        type=int,
    ...
    parser.add_argument(
        "--sfm-slr-rf-e", type=int, nargs="+", help="SelectFromModel rf n estimators"
    )
    parser.add_argument(
        "--sfm-slr-rf-d", type=str, nargs="+", help="SelectFromModel rf max depth"
    )
    parser.add_argument(
        "--sfm-slr-rf-f", type=str, nargs="+", help="SelectFromModel rf max features"
    )
    parser.add_argument(
        "--sfm-slr-ext-e", type=int, nargs="+", help="SelectFromModel ext n estimators"
    )
    parser.add_argument(
        "--sfm-slr-ext-d", type=str, nargs="+", help="SelectFromModel ext max depth"
    )
    parser.add_argument(
        "--sfm-slr-ext-f", type=str, nargs="+", help="SelectFromModel ext max features"
    )
    parser.add_argument(
        "--sfm-slr-grb-e", type=int, nargs="+", help="SelectFromModel grb n estimators"
    )
    parser.add_argument(
        "--sfm-slr-grb-lr",
        type=float,
    ...
    parser.add_argument(
        "--sfm-slr-grb-d", type=int, nargs="+", help="SelectFromModel grb max depth"
    )
    parser.add_argument(
        "--sfm-slr-grb-f", type=str, nargs="+", help="SelectFromModel grb max features"
    )
    parser.add_argument("--rlf-slr-n", type=int, nargs="+", help="ReliefF n neighbors")
    parser.add_argument("--rlf-slr-s", type=int, nargs="+", help="ReliefF sample size")
    parser.add_argument(
        "--ohe-trf-categories",
        type=str_list,
        nargs="+",
    ...
    parser.add_argument(
        "--ohe-trf-drop", type=str, choices=["first"], help="OneHotEncoder drop"
    )
    parser.add_argument(
        "--ord-trf-categories",
        type=str_list,
    ...
    parser.add_argument(
        "--mms-trf-feature-range",
        type=int_list,
        default=(0, 1),
        help="MinMaxScaler feature range",
    )
    parser.add_argument(
        "--log-trf-base", type=int, default=2, help="LogTransformer base"
    )
    parser.add_argument(
        "--log-trf-shift", type=float, nargs="+", help="LogTransformer shift"
    )
    parser.add_argument(
        "--pwr-trf-meth",
        type=str,
    ...
    parser.add_argument(
        "--nsn-trf-bg", type=str, nargs="+", help="NanoStringNormalizer background"
    )
    parser.add_argument(
        "--nsn-trf-bg-t",
        type=str_bool,
    ...
    parser.add_argument(
        "--svc-clf-cw", type=str, nargs="+", help="SVC/LinearSVC class weight"
    )
    parser.add_argument("--svc-clf-kern", type=str, nargs="+", help="SVC kernel")
    parser.add_argument("--svc-clf-deg", type=int, nargs="+", help="SVC poly degree")
    parser.add_argument("--svc-clf-g", type=str, nargs="+", help="SVC gamma")
    parser.add_argument(
        "--svc-clf-max-iter",
    ...
    parser.add_argument(
        "--lsvc-clf-loss", type=str, default="squared_hinge", help="LinearSVC loss"
    )
    parser.add_argument(
        "--lsvc-clf-max-iter", type=int, default=1000, help="LinearSVC max_iter"
    )
    parser.add_argument(
        "--lsvc-clf-tol", type=float, default=1e-2, help="LinearSVC tol"
    )
    parser.add_argument(
        "--svc-clf-cache", type=int, default=2000, help="SVC cache size"
    )
    parser.add_argument(
        "--lgr-clf-ce", type=int, nargs="+", help="LogisticRegression C exp"
    )
    parser.add_argument(
        "--lgr-clf-ce-min", type=int, help="LogisticRegression C exp min"
    )
    parser.add_argument(
        "--lgr-clf-ce-max", type=int, help="LogisticRegression C exp max"
    )
    parser.add_argument(
        "--lgr-clf-l1r", type=float, nargs="+", help="LogisticRegression l1 ratio"
    )
    parser.add_argument(
        "--lgr-clf-l1r-min", type=float, help="LogisticRegression l1 ratio min"
    )
    parser.add_argument(
        "--lgr-clf-l1r-max", type=float, help="LogisticRegression l1 ratio max"
    )
    parser.add_argument(
        "--lgr-clf-l1r-step",
        type=float,
    ...
    parser.add_argument(
        "--lgr-clf-cw", type=str, nargs="+", help="LogisticRegression class weight"
    )
    parser.add_argument(
        "--lgr-clf-dual",
        default=False,
    ...
    parser.add_argument(
        "--lgr-clf-max-iter", type=int, default=1000, help="LogisticRegression max_iter"
    )
    parser.add_argument(
        "--lgr-clf-verbose", type=int, default=0, help="LogisticRegression verbosity"
    )
    parser.add_argument(
        "--knn-clf-k", type=int, nargs="+", help="KNeighborsClassifier neighbors"
    )
    parser.add_argument(
        "--knn-clf-w", type=str, nargs="+", help="KNeighborsClassifier weights"
    )
    parser.add_argument(
        "--dt-clf-d", type=str, nargs="+", help="DecisionTreeClassifier max depth"
    )
    parser.add_argument(
        "--dt-clf-f", type=str, nargs="+", help="DecisionTreeClassifier max features"
    )
    parser.add_argument(
        "--dt-clf-cw", type=str, nargs="+", help="DecisionTreeClassifier class weight"
    )
    parser.add_argument(
        "--rf-clf-e", type=int, nargs="+", help="RandomForestClassifier n estimators"
    )
    parser.add_argument(
        "--rf-clf-d", type=str, nargs="+", help="RandomForestClassifier max depth"
    )
    parser.add_argument(
        "--rf-clf-f", type=str, nargs="+", help="RandomForestClassifier max features"
    )
    parser.add_argument(
        "--rf-clf-cw", type=str, nargs="+", help="RandomForestClassifier class weight"
    )
    parser.add_argument(
        "--ext-clf-e", type=int, nargs="+", help="ExtraTreesClassifier n estimators"
    )
    parser.add_argument(
        "--ext-clf-d", type=str, nargs="+", help="ExtraTreesClassifier max depth"
    )
    parser.add_argument(
        "--ext-clf-f", type=str, nargs="+", help="ExtraTreesClassifier max features"
    )
    parser.add_argument(
        "--ext-clf-cw", type=str, nargs="+", help="ExtraTreesClassifier class weight"
    )
    parser.add_argument(
        "--ada-clf-e", type=int, nargs="+", help="AdaBoostClassifier n estimators"
    )
    parser.add_argument(
        "--ada-clf-lgr-ce",
        type=int,
    ...
    parser.add_argument(
        "--grb-clf-d", type=int, nargs="+", help="GradientBoostingClassifier max depth"
    )
    parser.add_argument(
        "--grb-clf-f",
        type=str,
    ...
    parser.add_argument(
        "--mlp-clf-hls", type=str, nargs="+", help="MLPClassifier hidden layer sizes"
    )
    parser.add_argument(
        "--mlp-clf-act", type=str, nargs="+", help="MLPClassifier activation function"
    )
    parser.add_argument(
        "--mlp-clf-slvr", type=str, nargs="+", help="MLPClassifier solver"
    )
    parser.add_argument(
        "--mlp-clf-a", type=float, nargs="+", help="MLPClassifier alpha"
    )
    parser.add_argument(
        "--mlp-clf-lr", type=str, nargs="+", help="MLPClassifier learning rate"
    )
    parser.add_argument(
        "--sgd-clf-ae", type=int, nargs="+", help="SGDClassifier alpha exp"
    )
    parser.add_argument(
        "--sgd-clf-ae-min", type=int, help="SGDClassifier alpha exp min"
    )
    parser.add_argument(
        "--sgd-clf-ae-max", type=int, help="SGDClassifier alpha exp max"
    )
    parser.add_argument(
        "--sgd-clf-l1r", type=float, nargs="+", help="SGDClassifier l1 ratio"
    )
    parser.add_argument(
        "--sgd-clf-l1r-min", type=float, help="SGDClassifier l1 ratio min"
    )
    parser.add_argument(
        "--sgd-clf-l1r-max", type=float, help="SGDClassifier l1 ratio max"
    )
    parser.add_argument(
        "--sgd-clf-l1r-step",
        type=float,
    ...
    parser.add_argument(
        "--sgd-clf-cw", type=str, nargs="+", help="SGDClassifier class weight"
    )
    parser.add_argument(
        "--sgd-clf-loss",
        type=str,
    ...
    parser.add_argument(
        "--sgd-clf-max-iter", type=int, default=1000, help="SGDClassifier max_iter"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--deseq2-score-type", type=str, default="pv", help="DESeq2 score type"
    )
    parser.add_argument(
        "--deseq2-trans-type", type=str, default="vst", help="DESeq2 trans type"
    )
    parser.add_argument(
        "--deseq2-no-lfc-shrink",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "--edger-score-type", type=str, default="pv", help="edgeR score type"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limma-trans-type", type=str, default="cpm", help="limma trans type"
    )
    parser.add_argument(
        "--limma-no-log",
        default=False,
    ...
    parser.add_argument(
        "--limma-trend", default=False, action="store_true", help="limma trend"
    )
    parser.add_argument(
        "--limma-model-batch",
        default=False,
        action="store_true",
        help="limma model batch",
    )
    parser.add_argument(
        "--limma-model-dupcor",
        default=False,
    ...
    parser.add_argument("--scv-splits", type=int, default=10, help="scv splits")
    parser.add_argument("--scv-repeats", type=int, default=0, help="scv repeats")
    parser.add_argument("--scv-size", type=float, default=0.2, help="scv size")
    parser.add_argument("--scv-verbose", type=int, help="scv verbosity")
    parser.add_argument(
        "--scv-scoring",
        type=str,
        nargs="+",
        choices=["roc_auc", "balanced_accuracy", "average_precision"],
        default=["roc_auc", "balanced_accuracy", "average_precision"],
    ...
    parser.add_argument(
        "--scv-n-iter", type=int, default=100, help="randomized scv num iterations"
    )
    parser.add_argument(
        "--scv-error-score", type=str, default="nan", help="scv error score"
    )
    parser.add_argument(
        "--scv-use-ssplit",
        default=False,
    ...
    parser.add_argument("--test-repeats", type=int, default=0, help="num outer repeats")
    parser.add_argument(
        "--test-size", type=float, default=0.2, help="outer splits test size"
    )
    parser.add_argument(
        "--test-use-ssplit",
        default=False,
    ...
    parser.add_argument(
        "--title-font-size", type=int, default=14, help="figure title font size"
    )
    parser.add_argument(
        "--axis-font-size", type=int, default=14, help="figure axis font size"
    )
    parser.add_argument(
        "--long-label-names",
        default=False,
    ...
    parser.add_argument("--fig-width", type=float, default=10, help="figure width")
    parser.add_argument("--fig-height", type=float, default=10, help="figure height")
    parser.add_argument(
        "--fig-format",
        type=str,
        nargs="+",
    ...
    parser.add_argument(
        "--save-figs", default=False, action="store_true", help="save figures"
    )
    parser.add_argument(
        "--show-figs", default=False, action="store_true", help="show figures"
    )
    parser.add_argument(
        "--save-models", default=False, action="store_true", help="save models"
    )
    parser.add_argument(
        "--save-results", default=False, action="store_true", help="save results"
    )
    parser.add_argument("--save-model-code", type=str, help="save model code")
    ...
    parser.add_argument(
        "--max-nbytes", type=str, default="1M", help="joblib parallel max_nbytes"
    )
    parser.add_argument(
        "--cache",
        type=str,
    ...
    parser.add_argument(
        "--gbytes-limit", type=int, help="Joblib cache size limit in GB"
    )
    parser.add_argument(
        "--out-dir", type=dir_path, default=os.getcwd(), help="output dir"
    )
    parser.add_argument(
        "--tmp-dir", type=dir_path, default=gettempdir(), help="tmp dir"
    )
    parser.add_argument(
        "--clean-results-dir",
        default=False,
    ...
    parser.add_argument(
        "--random-seed", type=int, default=777, help="random state seed"
    )
    parser.add_argument(
        "--jvm-heap-size", type=int, default=500, help="rjava jvm heap size"
    )
    parser.add_argument(
        "--filter-warnings",
        type=str,
        nargs="+",
        choices=["convergence", "joblib", "fitfailed", "slr", "qda"],
    ...
    parser.add_argument(
        "--n-perms", type=int, default=1000, help="permutation test n permutations"
    )
    parser.add_argument("--perm-verbose", type=int, help="permutation test verbosity")
    parser.add_argument(
        "--pipe-verbose",
        default=False,
        action="store_true",
    ...
    if args.cache:
        ...
        bytes_limit = (
            args.gbytes_limit * 1024**3 if args.gbytes_limit is not None else None
        )
    ...
    if estm_memory:
        ...
        lsvc_clf = CachedLinearSVC(
            dual=False,
            max_iter=args.lsvc_clf_max_iter,
            memory=estm_memory,
            penalty="l1",
            random_state=args.random_seed,
        ...
        lgr_clf = CachedLogisticRegression(
            dual=args.sfm_slr_lgr_dual,
            max_iter=args.lgr_clf_max_iter,
            memory=estm_memory,
            penalty=args.sfm_slr_lgr_penalty,
            random_state=args.random_seed,
            solver=args.sfm_slr_lgr_solver,
        ...
        mui_clf_scorer = MutualInfoScorerClassification(random_state=args.random_seed)
        lsvc_clf = LinearSVC(
            dual=False,
            max_iter=args.lsvc_clf_max_iter,
            penalty="l1",
    ...
    if cv_params["col_slr_file"]:
        for feature_file in cv_params["col_slr_file"]:
            if not os.path.isfile(feature_file):
                raise IOError("File does not exist/invalid: {}".format(feature_file))
    ...
    for cv_param, cv_param_values in cv_params.copy().items():
        ...
        if cv_param in (
            ...
            if (
                cv_params["{}_min".format(cv_param)] == 1
                and cv_params["{}_step".format(cv_param)] > 1
            ):
                cv_params[cv_param] = np.array(
                    [1]
                    + list(
                        range(
                            0,
            ...
            cv_params[cv_param[:-1]] = np.logspace(
                cv_param_v_min,
                cv_param_v_max,
                cv_param_v_max - cv_param_v_min + 1,
                base=10,
            )
            ...
            cv_params[cv_param] = sorted(
                [None if v.title() == "None" else v for v in cv_param_values],
                key=lambda x: (x is None, x),
            )
        elif cv_param in (
            "sfm_slr_rf_d",
            ...
            cv_params[cv_param] = sorted(
                [None if v.title() == "None" else int(v) for v in cv_param_values],
                key=lambda x: (x is None, x),
            )
    ...
    pipe_config = {
        ...
        "SelectKBest-ANOVAFScorerClassification": {
            ...
            "param_grid": {"k": cv_params["skb_slr_k"]},
        },
        ...
        "DESeq2Selector": {
            "estimator": DESeq2Selector(
                ...
                score_type=args.deseq2_score_type,
        ...
        "DESeq2WrenchSelector": {
            "estimator": DESeq2WrenchSelector(
        ...
        "DESeq2ZINBWaVESelector": {
            "estimator": DESeq2ZINBWaVESelector(
                norm_type=args.deseq2_norm_type,
                fit_type=args.deseq2_fit_type,
                score_type=args.deseq2_score_type,
                trans_type=args.deseq2_trans_type,
        ...
        "DESeq2WrenchZINBWaVESelector": {
            "estimator": DESeq2WrenchZINBWaVESelector(
                est_type=args.wrench_est_type,
                ref_type=args.wrench_ref_type,
                z_adj=args.wrench_z_adj,
                fit_type=args.deseq2_fit_type,
        ...
        "EdgeRSelector": {
            "estimator": EdgeRSelector(
                ...
                score_type=args.edger_score_type,
        ...
        "EdgeRWrenchZINBWaVESelector": {
            "estimator": EdgeRWrenchZINBWaVESelector(
                est_type=args.wrench_est_type,
                ref_type=args.wrench_ref_type,
                z_adj=args.wrench_z_adj,
                score_type=args.edger_score_type,
        ...
        "LimmaVoomSelector": {
            "estimator": LimmaVoomSelector(
                norm_type=args.limma_norm_type,
                score_type=args.limma_score_type,
                trans_type=args.limma_trans_type,
                model_batch=args.limma_model_batch,
        ...
        "LimmaVoomWrenchSelector": {
            "estimator": LimmaVoomWrenchSelector(
                est_type=args.wrench_est_type,
                ref_type=args.wrench_ref_type,
                z_adj=args.wrench_z_adj,
                score_type=args.limma_score_type,
        ...
        "CFS": {"estimator": CFS()},
        # transformers
        "ColumnTransformer": {"estimator": ExtendedColumnTransformer([], n_jobs=1)},
        "OneHotEncoder": {
            "estimator": OneHotEncoder(
                categories=(
                    args.ohe_trf_categories if args.ohe_trf_categories else "auto"
                ),
                drop=args.ohe_trf_drop,
                handle_unknown=("ignore" if args.ohe_trf_drop is None else "error"),
                sparse=False,
            )
        },
        "OrdinalEncoder": {
            "estimator": OrdinalEncoder(
                categories=(
                    args.ord_trf_categories if args.ord_trf_categories else "auto"
                )
            )
        },
        "LogTransformer": {
            "estimator": LogTransformer(base=args.log_trf_base),
            "param_grid": {"shift": cv_params["log_trf_shift"]},
        },
        "PowerTransformer": {
            "estimator": PowerTransformer(),
            "param_grid": {"method": cv_params["pwr_trf_meth"]},
        },
        "MinMaxScaler": {
        ...
        "LogisticRegression": {
            "estimator": LogisticRegression(
                dual=args.lgr_clf_dual,
                max_iter=args.lgr_clf_max_iter,
                penalty=args.lgr_clf_penalty,
                random_state=args.random_seed,
                solver=args.lgr_clf_solver,
                verbose=args.lgr_clf_verbose,
            ),
            "param_grid": {
        ...
        "GaussianNB": {"estimator": GaussianNB(), "param_routing": ["sample_weight"]},
        ...
        "SGDClassifier": {
            "estimator": SGDClassifier(
                max_iter=args.sgd_clf_max_iter,
